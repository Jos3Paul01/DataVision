...
with open('PesosTreino/Nomes.names') as f:
    # cria uma lista com todos os nomes
    labels = [line.strip() for line in f]

# carrega os arquivos treinados pelo framework
network = cv2.dnn.readNetFromDarknet('PesosTreino/Config.cfg',
                                     'PesosTreino/Pesos.weights')
...
# Gera cores randomicas em cada objeto detectados
colours = np.random.randint(0, 255, size=(len(labels), 3), dtype='uint8')

# Loop de captura e detecção dos objetos

with open('DataVision/Resultados.txt', 'w') as arquivo:
